...
async def show_my_reports(event: types.Message | types.CallbackQuery, session: AsyncSession):
    ...
    if isinstance(event, types.CallbackQuery):
        # Используем try/except на случай, если сообщение идентично (чтобы бот не падал)
        try:
...
async def view_single_report(callback: types.CallbackQuery, session: AsyncSession):
    ...
    report_text = f"📋 **Отчет за {shift.closed_at.strftime('%d.%m.%Y')}**\n\n"
    total_sum = 0
...
@router.callback_query(F.data.startswith("confirm_del_rep:"))
async def delete_report_final(callback: types.CallbackQuery, session: AsyncSession):
    ...
    # Сбрасываем на 0 страницу, чтобы гарантированно увидеть список
